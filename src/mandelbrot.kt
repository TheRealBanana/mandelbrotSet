...
const val RESOLUTION_LIMIT_Y: Double = 0.001
const val RESOLUTION_LIMIT_X: Double = 0.001

//How many iterations should we run before we are certain of an escape velocity?
const val ESCAPE_VELOCITY_TEST_ITERATIONS: Int = 300
...
fun init(windowSizeW: Int = WINDOW_SIZE_WIDTH, windowSizeH: Int = WINDOW_SIZE_HEIGHT) {
    ...
    GL11.glOrtho(-2.2, 1.2, -1.2, 1.2, -1.0, 1.0)
...
fun findEscapeVelocity2(c: ComplexNumber): Double {
...
fun mandelbrotsimple(): Map<ComplexNumber, Color> {
    ...
    while (curImagCoord in -1.0..1.0) {
        ...
        while (real in -2.0..1.0) {
            ...
            val escapevelocity: Double = findEscapeVelocity2(ComplexNumber(real, curImagCoord))
            if (escapevelocity == 0.0) {
                ...
                //println("Velocity for point (${real}, ${curImagCoord}) is $escapevelocity")
                ...
                //Non-bounded value. Color by its velocity. Meh will do later
            }
